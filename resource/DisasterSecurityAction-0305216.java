...
import gov.fema.handler.ManageDisasterUsers;
...
public class DisasterSecurityAction extends BobjAction {
	...
	private static Logger log = Logger.getLogger(DisasterSecurityAction.class);
    ...
    private void populateDisasterSecurity(String strLogin, HttpServletRequest request,
        ...
        // To prevent running methods of already created ou/group in AD
        boolean bool = false;
        int msgCounter = 0;
        while(parameters.hasMoreElements()) {
            ...
            if (parameterValue.equals("on")) {
                ...
                if(user.size() == 0) {
                    ...
                    log.info("msgCounter : " + msgCounter);
                    if (msgCounter == 0) {
                    	message = message + userName + " Business Objects Service Account created.<BR>";
                    	msgCounter++;
                    } else {
                    	message = message + "<BR/>" + userName + " Business Objects Service Account created.<BR>";
                    }
                }
                else {
                        ...
                        while (ialias.hasNext()) {
                            ...
                            if (userAlias.isDisabled()) {
                                ...
                                if (msgCounter == 0) {
                                	...
                                	message = message + "<BR/>" +  userName + " Business Objects Service Account was unlocked.<BR>";
                                }
                            	...
                            	if (msgCounter == 0) {
                                	...
                                	message = message + "<BR/>" + userName + " Business Objects Service Account already exists and was not locked.<BR>";
                                }
            ...
            if (parameterValue.equals("on")) {
                ...
                // Create Group object
                log.info("=== OrganizationalUnit/Group Creation Started ===");
                ...
                log.info("=== OrganizationalUnit Creation Started ===");
                // Only need to be created once per Disaster Number
                if(Boolean.FALSE.equals(bool)) {
                	ldapDao.createOrganizationalUnit(group);
                	//message = message + group.getMessage() + " .<BR>";
                	log.info("=== Admin Group Creation Started ==="); 
                    ldapDao.createAdminGroup(group, Config.getProperty(ParameterConstants.BO_DISASTER_NUMBER_ONLY));
                    message = message + "<BR/>" + group.getMessage() + ".<BR>";
                    bool = true;
                }
                ldapDao.createAdminGroup(group, Config.getProperty(ParameterConstants.BO_DISASTER_NUMBER_PROGRAM));
                message = message + "<BR/>" + group.getMessage() + ".<BR>";
                log.info("=== Group Creation Started ==="); 
                ldapDao.createGroup(group);
                message = message + group.getMessage() + ".<BR>";
                log.info("=== Permission Control Started for Disaster Number only Admin ===");
                ldapDao.addGroupPermission(group, Config.getProperty(ParameterConstants.BO_DISASTER_NUMBER_ONLY));
                message = message + "<BR/>" + group.getMessage() + ".<BR>";
                log.info("=== Permission Control Started for Disaster Number and Disaster Program Admin ===");
                ldapDao.addGroupPermission(group, Config.getProperty(ParameterConstants.BO_DISASTER_NUMBER_PROGRAM));
                message = message + group.getMessage() + ".<BR>";
                log.info("=== OrganizationalUnit/Group Creation completed ===");
            }
        }
        userForm.setPageCounter("1m");
