			...
			if (groups != null && groups.size() > 0) {
				...
				for (Group adGroup : groupList) {
					...
					if (adGroup.getsAMAccountName().startsWith(Config.getProperty(ParameterConstants.BO_GROUP_USERS_PREFIX))) {
						...
						// Storing the group sAMAccountName into the disasterNumberReportType
						user.setDisasterNumberReportType(adGroup.getsAMAccountName());				
	...
	public List<User> getMemberOfListFromUser(String sAMAccountName) {
		...
		List<User> users = ldapTemplate.search(BASE_DN, filter.encode(), new AttributesMapper<User>() {
			...
			public User mapFromAttributes(Attributes attributes) throws NamingException { 
				...
				return new User(distinguishedName, null, 
					    displayName, cn, 
					    memberOfs, sAMAccountName, 
					    null, null, 
					    null,  email, disasterNumbers, reportTypes, disasterNumberReportTypes); 
			} 
		}); 
...
//				Attributes groupAttributes = new BasicAttributes();
	...
	public boolean createAdminOrganizationalUnit(Group group) {
		...
		try {
			...
			log.error("Admin OrganizationalUnit already exist!");
			return false;
		} catch (Exception e) {
			log.error(e, e);
			return false;
...
//			String groupDN = groupName + "," + "OU=" + group.getReportType() + "_Admins" + ",ou=" + group.getOu() + "," + Config.getProperty(ParameterConstants.BO_GROUP);
	...
	public boolean addGroupPermission(Group group, String adminType) {
		
		Process process = null;
		
		try {
			
			log.info("Testing running a simple command ...");
			dirList();
			...
			System.out.println("dsacls executable is not found!");
			return false;
