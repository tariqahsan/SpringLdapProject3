...
//				Attributes groupAttributes = new BasicAttributes();
	...
	public Group createAdminOrganizationalUnit(Group group) {
		...
		try {
			...
			group.setMessage(group.getOu().concat(" Admin Organizational Unit created") );
			return group;
		} catch (NameAlreadyBoundException nabex) {        	
			log.error(group.getOu().concat(" Admin OrganizationalUnit already exists!"));
			group.setMessage(group.getOu().concat(" Admin Organizational Unit already exists"));
			return group;
		} catch (Exception e) {
			log.error(e, e);
			group.setMessage("Error in creating " + group.getOu().concat(" Admin Organizational Unit"));
			return group;
		} 
	}
	...
	public Group createGroup(Group group)  {
		...
		try {
			...
			//groupAttributes.put("member", "CN=Wadsworth\\, Anthony,OU=BRF,OU=Users,OU=VANPSC,DC=fema,DC=net");
...
//			ldapTemplate.bind(groupName + "," + "OU=" + group.getReportType() + "_Admins" + ",ou=" + group.getOu() + "," + Config.getProperty(ParameterConstants.BO_GROUP), null, groupAttributes);
	...
	public boolean removeGroup(Group group) {
		try {
			...
			log.info("Deleting group : " + groupDN);
...
//			String groupDN = groupName + "," + "OU=" + group.getReportType() + "_Admins" + ",ou=" + group.getOu() + "," + Config.getProperty(ParameterConstants.BO_GROUP);
			...
			log.info("Deleting admin group : " + groupDN);
			ldapTemplate.unbind(groupDN);
			return true;
	...
	public boolean removeAdminOrganizationalUnit(Group group) {
		...
		try {
			...
			// Construct the ou name
			String ouName = "OU=" + group.getOu() + "," + Config.getProperty(ParameterConstants.BO_GROUP);
			log.info("Deleting organizational unit : " + ouName);
	...
	public Group addGroupPermission(Group group, String adminType) {
		...
		try {
			...
			// Check if the groups exists in AD
			boolean flag = true;
			while (flag) {
			...
			//TimeUnit.SECONDS(Integer.parseInt(Config.getProperty(ParameterConstants.SLEEP_TIME)));
			Thread.sleep(Integer.parseInt(Config.getProperty(ParameterConstants.SLEEP_TIME)));
			
			//log.info("Adding group permission to " + groupName + ",ou=" + group.getOu() + "," + Config.getProperty(ParameterConstants.BO_GROUP));
			...
			System.out.println("Adding permission to group");
			String dcaclsCommand = "cmd /C dsacls \"" + userGroupDn +  "\" /G FEMA\\" + adminGroupName + ":RPWP";
...
//			group.setMessage("Adding group permission on " + adminGroupDn + " is successful");
			//group.setMessage("Added administrative rights to " + adminGroupName);
			group.setMessage(adminGroupName + " Active Directory Group permissions added");
			return group;
		} catch (IOException ioEx) {
			log.error("dsacls executable is not found!");
//			group.setMessage("Adding group permission on " + adminGroupDn + " failed");
//			group.setMessage("Failed to add administrative right to " + adminGroupName );
	...
	/**
	 * This class prepares Group object after ldap search.
	 */
	private class GroupAttributesMapper implements AttributesMapper<Group> {
		...
		public Group mapFromAttributes(Attributes attributes) throws NamingException {
			...
			if (attributes.get("sAMAccountName") != null) {
				group.setsAMAccountName(attributes.get("sAMAccountName").get().toString());
			}
			if (attributes.get("distinguishedName") != null) {
				group.setDistinguishedName(attributes.get("distinguishedName").get().toString());
			}
