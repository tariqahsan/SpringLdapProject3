...
public class LdapRepositoryImpl implements LdapRepository {
	...
	public List<User> getGroupNameList(String sAMAccountName) {
		log.info("Running getGroupNameList(" + sAMAccountName + ")");
		...
		try {
			...
			if (groups != null && groups.size() > 0) {
				...
				for (Group adGroup : groupList) {
					//log.info("sAMAccountName : " + adGroup.getsAMAccountName());
					// Create an User object and set the group's sAMAccountName to it
					// NOTE : Not sure if other than sAMAccountName is needed to be populated 
					// in the User object
					User user = new User();
					...
					if (adGroup.getsAMAccountName().startsWith(Config.getProperty(ParameterConstants.BO_GROUP_USERS_PREFIX))) {
						log.info(adGroup.getsAMAccountName());
						user.setsAMAccountName(adGroup.getsAMAccountName());				
	...
	@Override
	public List<User> getGroupListByUser(String sAMAccountName) {
